 ...
 useEffect(() => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch deliveries
      const deliveriesRes = await fetch('http://localhost:5000/api/deliveries/company/deliveries', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (deliveriesRes.ok) {
        ...
        if (deliveriesData.success) {
          ...
          setStats(prev => ({ ...prev, totalDeliveries: deliveriesData.pagination?.total || 0 }))
          setLoading(prev => ({ ...prev, deliveries: false }))
        }
      }

      // Fetch drivers
      const driversRes = await fetch('http://localhost:5000/api/company/drivers', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (driversRes.ok) {
        ...
        if (driversData.success) {
          ...
          const online = driversData.data.filter((d: Driver) => d.isOnline && d.isActive).length
          setStats(prev => ({ 
            ...prev, 
      ...
      // Fetch statistics
      const statsRes = await fetch('http://localhost:5000/api/company/statistics', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (statsRes.ok) {
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            >
              <IconRefresh className={cn("h-5 w-5", (loading.stats || loading.deliveries || loading.drivers) && "animate-spin")} />
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Latest delivery requests</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            ...
            ) : drivers.filter(d => d.isOnline).slice(0, 3).map((driver) => (
              <div key={driver._id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="h-2 w-20 bg-gray-200 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${(count / Math.max(deliveries.length, 1)) * 100}%` }}
                      ></div>
                    </div>
                  </div>
                </div>
...
function getRoleGreeting(role: string): string {
  const greetings: Record<string, string> = {
    admin: 'Manage the entire platform and monitor system performance.',
    company_admin: 'Manage your company\'s riders and delivery operations.',
    driver: 'Track your deliveries and manage your schedule.',
  }
